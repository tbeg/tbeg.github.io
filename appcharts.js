//
// Executed once the DOM content is loaded
//
require([
    "dojo/ready", 
    "dojo/parser", 
    "dojo/_base/array", 
    "dijit/registry",
    "dojo/_base/lang",
    "dojo/request/xhr",
    "dojox/layout/ContentPane",
    "dijit/form/Textarea",
	"dojox/charting/Chart2D",
	"dojox/charting/themes/MiamiNice",
	"dojo/dom-construct",
	"dojo/dom"
    ], function(ready, parser, array, registry, lang, xhr, domConstruct, dom, create){
        ready(function(){
        
            //
            // Initialize tabs with the values of 'exdata' variable
            //
            function initMenu() {
                var menubar = registry.byId('navMenu');
            
                // Load chapters JSON file
                var deferred = xhr("./exdata.json", {
                    handleAs: "json",
                    preventCache: true
                }).then(function(exdata) {
                    // Process chapters to create the menu
                    array.forEach(exdata, function(provincie, i) {
                        // Create new menu for the chapter
                        var menu = new dijit.Menu({});
                        var menubaritem = new dijit.MenuBarItem({
                            label: provincie.name,
							provincie: provincie,
							onClick: function(event) {
								if(provincie.external) {
									window.open(provincie.url1, provincie.name);
									return;
								}
								
								var tabcontainer = registry.byId('receiptTabs');
								if(!isReceiptTabOpened(provincie.name)) {
										
									var nestedtab = new dijit.layout.TabContainer({
										title: provincie.name,
										closable: true,
										nested: true
									});								
									
									var resultPane = new dojox.layout.ContentPane({
										title: "Kaartje", 
										closable: false,
										preventCache: true,
										href: provincie.url1
									});
									
									var codePane = new dijit.layout.ContentPane({
										title: "code", 
										closable: false,
										preventCache: true,
										receiptUrl: provincie.url2,
										style: "overflow: auto; width: 100%; height: 100%;",
										onShow: function() {
											if(!this.sourceCode) {
												xhr(this.receiptUrl, {
													handleAs: "text"
												
												}).then(function(obj){
													// do something with the obj 
													// chart.addSeries("y", new dojox.charting.StoreSeries(obj, { query: {} }, "totalPassed"));
													console.log(obj)										

												});
												
											}
										}
									});

									// resultPane.attr('content', dojo.create('div', {'innerHTML':'map'}))
									
									nestedtab.addChild(resultPane);
									nestedtab.addChild(codePane);
									tabcontainer.addChild(nestedtab);
									tabcontainer.selectChild(nestedtab);
								} else {
									var receiptTab = getReceiptTab(provincie.name);
									if(receiptTab) {
										tabcontainer.selectChild(receiptTab);
									}
								}
							}
							
                        });
                    

                        menubar.addChild(menubaritem);
                    });
                        
                    registry.byId('borderContainer').resize();
                },
                function(error) {
                    var dialog = new dijit.Dialog({
                        title: "Error !!!",
                        content: "Sorry there was an error loading the JSON file with chapters.",
                        style: "width: 200px"
                    });
                    dialog.show();
                });
            }
            
            //
            // Check if the receipt tab with the given name is opened.
            //
            function isReceiptTabOpened(name) {
                var tabcontainer = registry.byId('receiptTabs');
                var recipes = tabcontainer.getChildren();
                for(var i=0; i< recipes.length; i++) {
                    if(recipes[i].get('title') == name) {
                        return true;
                    }
                }
                return false;
            }
            
            //
            // Given a name returns a reference to the corresponding
            // receipt tab.
            //
            function getReceiptTab(name) {
                var tabcontainer = registry.byId('receiptTabs');
                var recipes = tabcontainer.getChildren();
                for(var i=0; i< recipes.length; i++) {
                    if(recipes[i].get('title') == name) {
                        return recipes[i];
                    }
                }
                return null;
            }
        
            // Initialize main menu
            initMenu();
        });
    });
